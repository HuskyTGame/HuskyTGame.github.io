...
相关配置文件，提供 [网盘](https://pan.baidu.com/s/1VhFU2ZAeG4oa8JJHUshyyg)（提取码：bxnt）下载
...
需要配置  ***glm*** (OpenGL 的数学库)：从 [官网](https://glm.g-truc.net/0.9.8/index.html) 或 [网盘](https://pan.baidu.com/s/1VhFU2ZAeG4oa8JJHUshyyg)（提取码：bxnt） 中下载，将下载好的 *glm* 文件夹放入之前配置的 include 文件夹中，如下图所示：
...
配置步骤详见此篇博文：[**Assimp的安装编译及使用过程全纪录**](https://www.jianshu.com/p/4f3a1271ce0b)

**步骤2.**配置单头文件图像加载库 ***stb_image.h*** ：

在 [github](#define STB_IMAGE_IMPLEMENTATION
#include "stb_image.h") 上下载一个头文件，放入项目文件夹内（项目文件夹在工程文件夹下），然后在 VS 中手动添加该文件；新建一个 c++ 文件，输入以下代码：
