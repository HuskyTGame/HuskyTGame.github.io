...
namespace ResLoadFramework
{
    public class BuildAssetBundleEditor
    {
        ...
        private static string PATH_AB_CONFIG_BINARY = "Assets/GameData/Data/ABData/AssetBundleConfig.bytes";
        #endregion
        /// <summary>
        /// 缓存  类型二 —— 基于文件夹进行打包的
        /// Key：ABName
        ...
        public static void Build()
        {
            ...
            CreateFile(PATH_AB_CONFIG_BINARY);
            AssetDatabase.Refresh();//此处是为了AssetBundleConfig生成而刷新
            ...
            #region 清除AB包名（防止meta文件变动导致Git需要提交）
        ...
        private static bool ContainedByDynamicLoadABPathList(string fullPath)
        {
            ...
            {
                //类型一为==；类型二为Contains
                if (fullPath.Contains(mDynamicLoadABPathList[i]))
                    return true;
            }
            return false;
        }
        private static void CreateDirectory(string folderFullPath)
        {
            if (Directory.Exists(folderFullPath) == false)
            {
                ...
                Debug.LogFormat("创建AB包文件夹成功，文件夹路径：{0}", folderFullPath);
            }
            else
                Debug.LogFormat("AB包文件夹已存在，文件夹路径：{0}", folderFullPath);
        }
        private static void CreateFile(string fullPath)
        {
            if (File.Exists(fullPath) == false) ToBinary<object>(new AssetBundleConfig(), fullPath);
        }
    }
}
````
