...
### 1.AssetBundle管理类 ``AssetBundleMgr``
...
**ResourceItem** 资源项数据类：存储 AssetBundle 信息，供加载资源使用。
...
public class AssetBundleMgr : Singleton<AssetBundleMgr>
    {
        ...
        public bool LoadABConfig()
        {
            ...
            for (int i = 0; i < config.ABDataList.Count; i++)
            {
                ...
                {
                    Debug.LogErrorFormat("AB包配置表加载、缓存出错！Crc重复，AssetName：{0}；ABName：{1}", resItem.AssetName, resItem.ABName);
                    return false;
                }
